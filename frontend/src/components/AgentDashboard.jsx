...
      
      // 🔥 UPDATE: Expecting 'phoneNumber' and 'isEmployee' from callController
      if (callData && callData.phoneNumber) {
        setIncomingCalls(prevCalls => [
          { 
            ...callData, 
            id: Date.now(), 
            caller: callData.phoneNumber, // Map backend phoneNumber to expected caller property
            isEmployee: callData.isEmployee || false // Include the routing flag, default to false
          },
          ...prevCalls 
        ]);
      } else {
        console.warn("Received incomplete call data, ignoring:", callData);
      }
    });
...
    // We need caller (phone number) and isEmployee flag for routing
    const phoneNumber = acceptedCall.caller; 
    const dashboardLink = acceptedCall.dashboardLink;
    const isEmployee = acceptedCall.isEmployee;

    if (!phoneNumber) {
      console.error("AgentDashboard: Cannot accept call. Missing caller phone number.", acceptedCall);
      return;
    }
...
    // 🔥 UPDATE: Implement conditional routing based on isEmployee flag
    if (isEmployee) {
      // If employee, redirect to the specific helpdesk path
      redirectUrl = `${BACKEND_URL}/employee-helpdesk?phoneNumber=${encodedPhoneNumber}`;
      console.log(`AgentDashboard: Employee call accepted. Redirecting to HELP DESK: ${redirectUrl}`);
    } else if (dashboardLink) {
      // If customer, redirect to the standard dashboard link provided by the backend
      redirectUrl = `${dashboardLink}?phoneNumber=${encodedPhoneNumber}`;
      console.log(`AgentDashboard: Customer call accepted. Redirecting to USER DASHBOARD: ${redirectUrl}`);
    } else {
      console.error("AgentDashboard: Cannot redirect. Missing dashboardLink for customer call.", acceptedCall);
      return;
    }
      
...
  };
  
  // Handles logging out the agent (unchanged)
