...
export function ServiceManSelectionPage() {
    ...
    // 💡 MODIFICATION: Added orderId to destructuring
    const { ticketId, orderId, requestDetails, selectedAddressId, serviceName, phoneNumber } = location.state || {};
    ...
    /**
     * UPDATED: Sends dispatch data to the backend.
     */
    const handleDispatch = async () => {
        
        // 🛑 FIX 1: Check if a serviceman is selected
        if (!selectedServiceman) {
            setDispatchStatus('Error: Please select a serviceman to dispatch.');
            setTimeout(() => setDispatchStatus(null), 3000);
            return;
        }

        // 🛑 FIX 2: Check for critical missing data fields
        if (!phoneNumber) {
            setDispatchStatus('Error: Cannot dispatch. Phone number data is missing.');
            setTimeout(() => setDispatchStatus(null), 5000);
            return;
        }
        if (!ticketId || !orderId) { // 🚀 UPDATED Check
            setDispatchStatus('Error: Cannot dispatch. Ticket ID or Order ID is missing.');
            setTimeout(() => setDispatchStatus(null), 5000);
            return;
        }
        ...
        const dispatchData = {
            ...
            // 💡 CONFIRMED: Included ticket_id, order_id and phone_number in the payload
            ticket_id: ticketId,
            order_id: orderId, // 🚀 NEW: Passed order_id to backend
    ...
    // 🚀 UPDATED: Safety check includes orderId
    if (!ticketId || !orderId || !selectedAddressId || !serviceName) {
        return (
            <div style={{ ...styles.container, justifyContent: 'center', alignItems: 'center' }}>
                <h1 style={{ color: '#ef4444' }}>Error: Navigation Data Missing</h1>
                <p style={{ color: '#6b7280' }}>
                   Ticket: {ticketId}, Order: {orderId}
                </p>
        ...
        <div style={styles.container}>
            ...
            <div style={styles.mainContent}>
                ...
                <div style={styles.card}>
                    <h2 style={{ fontSize: '1.25rem', fontWeight: '700', color: '#1f2937', marginBottom: '16px', borderBottom: '1px solid #e5e7eb', paddingBottom: '8px' }}>
                         Active Call Summary
                    </h2>
                    
                    {/* 🚀 NEW: Order & Ticket Display Block (Columnar look) */}
                    <div style={{ display: 'flex', gap: '40px', marginBottom: '16px', backgroundColor: '#f9fafb', padding: '12px', borderRadius: '8px' }}>
                        
                        {/* Column 1: Order Context (The "Table" Requirement) */}
