...
  const navigate = useNavigate();
  // 🔑 NEW: Retrieve state from navigation (passed by the component that handled the socket redirect)
  const location = useLocation();
  // We assume the parent component passes { state: { phoneNumber: '+1234567890' } }
  const initialPhoneNumber = location.state?.phoneNumber || 'Unknown/Missing'; 
...
  const [subscriptionStatus] = useState('Premium'); 
  // 🔑 NEW STATE: Use the phone number retrieved from navigation state
  const [userPhoneNumber] = useState(initialPhoneNumber);
  
...
    }
    
    // 🚨 CHECK 1: Require an address to be selected before proceeding
...
    }
    ...
    // 🚨 CHECK 2: Validate the phone number from state
    if (!userPhoneNumber || userPhoneNumber === 'Unknown/Missing') {
        setSaveMessage('Error: Phone number missing. Cannot create ticket.');
        setTimeout(() => setSaveMessage(''), 3000);
        return;
    }
...
    try {
      // 🔑 FIX: Use the actual phone number from component state (which came from location.state)
...
        },
        body: JSON.stringify({
          // 🔑 FIXED: Use the correct phone number state
...
          selectedAddressId: selectedAddressId, 
          // 🔑 NEW: Pass the correct phone number to the next screen if needed
          phoneNumber: phoneNumberToSend 
        }
      });
      
...
            
            {/* 🔑 NEW ROW: Display Phone Number from navigation state */}
            <div style={styles.infoRow}>
              <span style={styles.infoKey}>Phone Number</span>
              <span style={styles.infoVal}>{userPhoneNumber}</span>
            </div>
...
                        <div 
                            // Assuming 'id' exists on the address object for keying and selection
                            key={address.id} 
                            style={{ 
                                ...styles.addressItem, 
...
              // Disable if saving OR if addresses are available but none is selected
